    ...
    (farmsToDisplay) => {
      ...
      farmsToDisplay.map((farm) => {
        if(!farm.isTokenOnly){
          ...
          const { cakeRewardsApr } = getFarmApr(new BigNumber(farm.poolWeight), cakePrice, totalLiquidity, farm.lpAddresses[ChainId.MAINNET], new BigNumber(farm.xhashperblock))
          apy = cakeRewardsApr
    ...
    <StyledCakeStats>
      <Link href="/farms" style={{width: '100%', textDecoration:'none'}}>
       <CardBody style={{width: '100%'}}>
        ...
        <Row>
          <Text fontSize="36px" style={{color:'#daa827', fontWeight: 600}} >{getHighestAPY() ? (
            `${getHighestAPY()}%`
          ) : (
