...
type PublicFarmData = {
  ...
  multiplier: string
  depositfee: string
  xhashperblock: SerializedBigNumber
  canharvest: boolean
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, xhashPerBlock] =
    pid || pid === 0
  ...
  const XhashPerBlock = xhashPerBlock ? new BigNumber(xhashPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    xhashperblock: XhashPerBlock.toJSON(),
    canharvest
  }
}
